Shader "Custom/BRDF_KC" 
{
	Properties {
		...
		_EmuMap ("EmuMap", 2D) = "white" {}
		_EavgMap ("EavgMap", 2D) = "white" {}
	}
	...
	sampler2D _EmuMap;
	sampler2D _EavgMap;
	float MultiScatter(float roughness, float albedo, float NdotV, float NdotL)
	{
		float E_o = tex2D(_EmuMap, float2(NdotV, roughness));
		float E_i = tex2D(_EmuMap, float2(NdotL, roughness));
		float E_avg = tex2D(_EavgMap, float2(0.5, roughness));
		float f_ms = (1-E_i)*(1-E_o) / (PI * (1-E_avg));

		// copper
		float3 edgetint = float3(0.827, 0.792, 0.678);
		float3 F_avg = AverageFresnel(albedo, edgetint);
		float f_add = F_avg*E_avg / (1 - F_avg*(1-E_avg));
	...
	float whiteFrance(float3 n, float3 v,float roughness, float albedo)
	{
		...
		for (int i = 0; i < SAMPLER; i++)
		{
		    ...
		    float fms = MultiScatter(roughness, albedo, ndotv, ndotl);
			float BRDF = numerator / denominator + fms;
	...
	{   
		...
		float fms = MultiScatter(_Roughness, _Albedo, NdotV, NdotL);
		float3 BRDF = numerator / denominator + fms;
